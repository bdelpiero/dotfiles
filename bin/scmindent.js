...
function indentLine(currLine) {
  ...
  while (i < currLine.length) {
    ...
    if (escapeP) {
      ...
      parenStack.unshift({
        spacesBefore: 1 + i + currLeftI + si.deltaIndent,
...
function customize() {
  fs.readFile(process.env.LISPWORDS || process.env.HOME + "/lispwords.json", "utf8", function (err, data) {
    if (!err) {
